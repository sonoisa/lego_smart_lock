...
#
    ...
    def __init__(self, address=None, backend='BlueZ', controller='hci0'):
        self.address = address
        self._angle_sensor_callback_A = None
        self._angle_sensor_callback_B = None
        self._angle_sensor_callback_C = None
        self._angle_sensor_callback_D = None
        self._colordist_sensor_callback_C = None
        self._colordist_sensor_callback_D = None
    ...
    def start(self, timeout=1, retry=10):
        self.adapter.start()
        if self.address is not None:
            self.device = self.adapter.connect(self.address)
        else:
            for i in range(retry):
                devices = self.adapter.filtered_scan(run_as_root=True, name_filter="LEGO Move Hub", timeout=timeout)
                self.adapter.reset()
                for device in devices:
                    self.address = device['address']
                    self.device = self.adapter.connect(self.address)
                    return
            raise pygatt.exceptions.BLEError("LEGO Move Hub not found")
...
#
    def set_hublight(self, color):
        if color in LED_COLORS:
            self.device.char_write_handle(MOVE_HUB_HARDWARE_HANDLE, SET_LED_COLOR[LED_COLORS.index(color)])
...
# i.e. turn on
#
    def run_motor_constant(self, motor, dutycycle_pct):
        if motor in MOTORS:
            if dutycycle_pct in range(-100, 101):
                command = MOTOR_CONST_INI
                command += motor
                command += MOTOR_CONST_MID
                if dutycycle_pct < 0:
                    dutycycle_pct += 255
                command += bytes(bytes(chr(dutycycle_pct), 'latin-1'))
                command += MOTOR_CONST_END
        ...
        if handle == MOVE_HUB_HARDWARE_HANDLE:
            ...
            if value[0] == 0x08 and \
                ...
                if value[3] == PORT_A:
                    ...
                    if self._angle_sensor_callback_A is not None:
                        self._angle_sensor_callback_A(self.last_angle_A, old_value)

                elif value[3] == PORT_B:
                    # It's an Encoded Motor

                    old_value = self.last_angle_B
                    ...
                    if self._angle_sensor_callback_B is not None:
                        self._angle_sensor_callback_B(self.last_angle_B, old_value)
                    ...
                    if self._port_C_is == TYPE_COLORDISTANCE:
                        last_color = self.last_color_C
                        last_dist = self.last_distance_C
                        if value[4] != 0xFF:
                        ...
                        if self._colordist_sensor_callback_C is not None and (last_color != self.last_color_C or last_dist != self.last_distance_C):
                            self._colordist_sensor_callback_C(self.last_color_C, self.last_distance_C, last_color, last_dist)

                    elif self._port_C_is == TYPE_ENCODERMOTOR:

                        old_value = self.last_angle_C
                        ...
                        if self._angle_sensor_callback_C is not None:
                            self._angle_sensor_callback_C(self.last_angle_C, old_value)
                    ...
                    if self._port_D_is == TYPE_COLORDISTANCE:
                        last_color = self.last_color_D
                        last_dist = self.last_distance_D
                        if value[4] != 0xFF:
                        ...
                        if self._colordist_sensor_callback_D is not None and (last_color != self.last_color_D or last_dist != self.last_distance_D):
                            self._colordist_sensor_callback_D(self.last_color_D, self.last_distance_D, last_color, last_dist)

                    elif self._port_D_is == TYPE_ENCODERMOTOR:
                        old_value = self.last_angle_D
                        ...
                        if self._angle_sensor_callback_D is not None:
                            self._angle_sensor_callback_D(self.last_angle_D, old_value)
...
#
    def listen_colordist_sensor(self, port, callback=None):
        if port in [PORT_C, PORT_D]:
            ...
            if port == PORT_C:
                ...
                self._colordist_sensor_callback_C = callback
            else:
                self._port_D_is = TYPE_COLORDISTANCE
                self._colordist_sensor_callback_D = callback
...
#
    def listen_angle_sensor(self, port, callback=None):
        if port in [PORT_A, PORT_B, PORT_C, PORT_D]:
            ...
            if port == PORT_C:
                ...
                self._angle_sensor_callback_C = callback
            elif port == PORT_D:
                self._port_D_is = TYPE_ENCODERMOTOR
                self._angle_sensor_callback_D = callback
            elif port == PORT_A:
                self._angle_sensor_callback_A = callback
            elif port == PORT_B:
                self._angle_sensor_callback_B = callback
